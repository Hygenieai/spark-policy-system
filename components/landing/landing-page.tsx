...
/*
  ...
  Multi-step intake flow:
    Step 1 — Intake form (practice details)
    Step 2 — Square checkout (card entry + order summary)
    Step 3 — Confirmation (post-payment success state)
*/
...
type FlowStep = "intake" | "checkout" | "confirmed";
...
export default function LandingPage() {
  ...
  function handleIntakeSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    // Scroll the intake section into view smoothly
    window.setTimeout(
    ...
    <header className="nav-glass">
      <div className="container flex h-full items-center justify-between gap-6">
        <a
          ...
          aria-label="Spark Policy System home"
        >
        ...
        >
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--muted-fg)] transition hover:text-[var(--navy)]"
            >
              {link.label}
            </a>
          ))}
        </nav>
        <a href="#intake" className="primary-button hidden sm:inline-flex">
          Get the package
        </a>
      </div>
    </header>
      ...
      <div className="container relative grid items-center gap-14 lg:grid-cols-[0.92fr_1.08fr]">
        <div>
          ...
          <h1 className="display mt-7 max-w-4xl text-[clamp(3rem,6vw,5.25rem)] text-[var(--navy)]">
            A complete policy
            <br />
            and procedure <span className="gold-italic">system</span>
            <br />
            for physician practices.
          </h1>
          <p className="mt-7 max-w-xl text-lg leading-8 text-[var(--muted-fg)]">
            Standardize operations, clarify ownership, and install a polished policy
            structure your team can actually use. Built as a premium,
            ready-to-customize manual for modern physician practices.
          </p>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            <a href="#intake" className="primary-button">
              Get the Policy Package
            </a>
            <a href="#sample" className="secondary-button">
              View Sample Manual
            </a>
          </div>
          <div className="mt-9 flex flex-wrap gap-x-4 gap-y-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--muted-fg)]">
            {["One-time purchase", "Delivered as PDF", "Customized to your practice"].map(
              (item, index) => (
                <span
                  key={item}
                  className={
        ...
        <div className="relative lg:pl-6">
          <div
            className="absolute -left-3 -top-5 bottom-10 right-12 hidden border border-[var(--gold)]/45 lg:block"
            aria-hidden="true"
          />
          <PaperSurface className="relative overflow-hidden rounded-sm p-3">
            <Image
              src={heroImageUrl}
              alt="Physician and practice manager reviewing a policy manual"
              width={1408}
              height={1056}
              priority
          ...
          <div className="paper absolute -bottom-8 right-6 hidden w-72 p-6 lg:block">
            ...
            <p className="display mt-2 text-3xl text-[var(--navy)]">
              Policy & Procedure
            </p>
            <div className="my-4">
              <GoldRule />
            </div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--muted-fg)]">
              184 policies · 7 sections · single PDF
            </p>
          </div>
        </div>
      </div>
    ...
    <section id="value" className="section-pad">
      <div className="container">
        <SectionHeading
          ...
          title="Why practices use a system like this."
          italic="system"
          intro="A strong policy manual is not just documentation. It is operational infrastructure for how the practice makes decisions, trains staff, and protects consistency."
        />
        <div className="grid-divider mt-14 grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
          {valueProps.map((item) => (
            <article
              key={item.n}
              className="min-h-72 p-8 transition hover:bg-[#fbf7ee]"
            >
              <p className="display text-5xl text-[var(--gold)]">{item.n}</p>
              <h3 className="display mt-10 text-3xl text-[var(--navy)]">
                {item.title}
              </h3>
              <p className="mt-4 text-sm leading-7 text-[var(--muted-fg)]">
                {item.body}
              </p>
            </article>
          ))}
    ...
    <section id="sample" className="section-pad">
      <div className="container grid gap-12 lg:grid-cols-12">
        <div className="lg:sticky lg:top-28 lg:col-span-4 lg:self-start">
          <SectionHeading
            eyebrow="Sample Page"
            title="A sample policy page."
            italic="sample"
            intro="The manual is designed to feel deployable: each policy has an owner, approver, review cycle, and clear procedural steps."
          />
          <a
            href="#intake"
            className="mt-8 inline-block border-b border-[var(--gold)] pb-1 text-[11px] font-bold uppercase tracking-[0.22em] text-[var(--navy)]"
          >
            Get the full manual
          </a>
          <PaperSurface className="mt-10 overflow-hidden p-2">
            <Image
              src={manualImageUrl}
              alt="Stacked policy manual and practice operations documents"
              width={1408}
              height={1056}
        ...
        <div className="lg:col-span-8">
          <PaperSurface className="overflow-hidden p-8 md:p-12 lg:p-14">
            ...
            <div className="mt-8 flex flex-wrap justify-between gap-4 text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--muted-fg)]">
              <span>Spark Policy System · Edition I</span>
              <span>Section II.04</span>
            </div>
            <p className="mt-10 text-[11px] font-bold uppercase tracking-[0.28em] text-[var(--gold)]">
              Clinical Operations
            </p>
            <h3 className="display mt-3 text-[clamp(2rem,4vw,3.6rem)] text-[var(--navy)]">
            ...
            <div className="grid gap-px bg-[var(--border)] md:grid-cols-2">
              {[
                ...
                <div key={label} className="bg-[#fdfbf6] p-5">
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--muted-fg)]">
                    {label}
                  </p>
                  <p className="mt-2 text-sm leading-6 text-[var(--navy)]">{value}</p>
                </div>
              ))}
            </div>
            <div className="my-9">
            ...
            <div className="grid gap-8 md:grid-cols-2">
              <Signature label="Department Owner — Signature" />
              <Signature label="Approver — Signature & Date" />
            </div>
            <div className="mt-10 flex justify-between text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--muted-fg)]">
              <span>spark policy system</span>
...
function Signature({ label }: { label: string }) {
  return (
    <div>
      <div className="h-px bg-[var(--navy)]/35" />
      <p className="mt-3 text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--muted-fg)]">
        {label}
      </p>
    </div>
  );
    ...
    <section id="how" className="section-pad bg-[var(--slate-blue)] text-white">
      <div className="container">
        <div className="max-w-3xl">
          <p className="text-[11px] font-bold uppercase tracking-[0.28em] text-[var(--gold)]">
            How It Works
          </p>
          <h2 className="display mt-5 text-[clamp(2.25rem,4.2vw,3.75rem)]">
            From purchase to practice-ready manual.
          </h2>
        </div>
        <div className="mt-14 grid gap-px bg-white/15 md:grid-cols-3">
          {steps.map((step) => (
            <article key={step.n} className="bg-[var(--slate-blue)] p-8 md:p-10">
              <div className="grid h-14 w-14 place-items-center rounded-full border border-[var(--gold)] text-[var(--gold)]">
                <span className="display text-2xl">{step.n}</span>
              </div>
              <h3 className="display mt-10 text-3xl">{step.title}</h3>
...
// ── Intake / checkout / confirmation section ──────────────────────────────────
...
type IntakeSectionProps = {
  ...
  onIntakeSubmit: (event: React.FormEvent<HTMLFormElement>) => void;
        ...
        {flowStep === "intake" && (
          <div className="mx-auto max-w-4xl">
            ...
            <div className="mb-10 flex items-center gap-4">
              <StepBadge n="01" label="Practice details" active />
              <div className="h-px flex-1 bg-[var(--border)]" />
              <StepBadge n="02" label="Payment" />
              <div className="h-px flex-1 bg-[var(--border)]" />
            ...
            <SectionHeading
              eyebrow="Begin"
              title="Customize your manual."
              italic="your manual"
              intro="Tell us where the first edition should be pointed. We will prepare a customized manual and guide you through checkout."
              align="center"
            />
            <PaperSurface className="mt-12 p-7 md:p-10">
              <form onSubmit={onIntakeSubmit} className="grid gap-8 md:grid-cols-2">
                ...
                <Field label="Department owner / signer" required>
                  <input
                    className="field-input"
                    required
                ...
                <div className="flex flex-col gap-5 border-t border-[var(--border)] pt-7 md:col-span-2 md:flex-row md:items-center md:justify-between">
                  <p className="max-w-lg text-xs leading-6 text-[var(--muted-fg)]">
                    This product is not legal advice. It is a document system for
                    operational use and does not collect PHI.
                  </p>
  ...
  return (
    <PaperSurface className="mx-auto max-w-2xl p-8 text-center md:p-12">
      ...
      <div className="mb-10 flex items-center justify-center gap-4">
        ...
        <StepBadge n="02" label="Payment" done />
        <div className="h-px w-12 bg-[var(--border)]" />
        <StepBadge n="03" label="Confirmation" active />
      </div>
      ...
      <h2 className="display mt-3 text-[clamp(2.25rem,4vw,3.75rem)] text-[var(--navy)]">
        Your manual is on its <span className="gold-italic">way.</span>
      </h2>
      <p className="mx-auto mt-5 max-w-lg text-base leading-8 text-[var(--muted-fg)]">
        ...
        <span className="font-semibold text-[var(--navy)]">
          {values.buyerEmail || "your email"}
        </span>{" "}
        once the production workflow is connected.
      </p>

      <div className="mt-9 border border-[var(--border)] bg-[#fdfbf6] p-6 text-left">
        <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-[var(--muted-fg)]">
          What happens next
        </p>
        <div className="mt-5 space-y-4">
          {[
            "Square confirms the payment and issues a receipt.",
            "Supabase stores the intake request and order record.",
            "The manual generation workflow prepares the customized PDF.",
            "Your practice-ready manual arrives by email.",
          ].map((item, index) => (
            <p key={item} className="flex gap-4 text-sm text-[var(--navy)]">
      ...
      {paymentToken && (
        <p className="mt-6 break-all text-[10px] font-mono text-[var(--muted-fg)]">
          Payment token: {paymentToken}
        </p>
      )}

      <button type="button" onClick={onReset} className="secondary-button mt-8">
        Return to site
      </button>
    </PaperSurface>
  );
...
function ClosingCta() {
  return (
    <section className="bg-[rgba(234,228,214,0.50)] py-24 text-center">
      <div className="container">
        <SectionHeading
          eyebrow="A One-Time Purchase"
          title="Install a polished policy structure in a single afternoon."
          italic="single afternoon"
          align="center"
        />
        <a href="#intake" className="primary-button mt-9">
          Begin the intake
        </a>
      </div>
    </section>
...
function Footer() {
  return (
    <footer className="border-t border-[var(--border)] py-10">
      <div className="container flex flex-col justify-between gap-4 text-[11px] font-semibold uppercase tracking-[0.20em] text-[var(--muted-fg)] md:flex-row">
        <p>Spark Policy System · Edition I, Dermatology</p>
        <p>
          © {new Date().getFullYear()} Spark. Operational document system; not legal
          advice.
        </p>
      </div>
    </footer>
